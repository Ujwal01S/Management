...
import ObjectAndShadTab from "./components/jsObjectProperty/objectAndShadTab";
import { Checkbox } from "./components/ui/checkbox";
import { useState } from "react";
import ChartForGender from "./components/piechart/piechart";
import PassingPropsToChildElemnt from "./components/passingProptoHtmlELemet";
import CarouselDemo from "./components/crousal";
import DialogStateMangement from "./components/dialogStateManagment";
import ArrayMethod from "./components/arrayMethod";
import UserProvider from "./store/user-context";
import TestingContextApi from "./testingContextApi/testingContextApi";
...
function App() {
  ...
  //   }else{
  //     console.log('Unchecked is done')
  //   }
  // };

  const [childrenElementBoolean, setChildrenElementBoolean] =
    useState<boolean>(false);

  return (
    <>

    {/* Checking out creation of new array from existing array of object and adding new property */}
      ...
      {/* <DialogStateMangement /> */}

      {/* passing props */}
      {/* <PassingPropsToChildElemnt
        className="text-red-700"
        childrenElementBoolean={childrenElementBoolean}
        onClick={() => {
          console.log("clg triggered from parent to child random element");
          setChildrenElementBoolean((prev) => !prev);
        }}
        title="This is demo to pass props to tag of children component"
      >
        <p className={`${childrenElementBoolean ? "text-blue-700" : ""}`}>
          This elemnt color is being changed based on above p that is in another
          component
        </p>
      </PassingPropsToChildElemnt> */}
      ...
      {/* This part here contains cva twMerge svg before and after */}

      {/* < className="flex flex-col justify-center items-center py-4 gap-8">
